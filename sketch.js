...
window.onload = function() {
  ...
  datGui.add(myData, 'value0', 0, 255).listen();
  datGui.add(myData, 'value1', 0, 255).listen();
  datGui.add(myData, 'value2', 0, 255).listen();
  datGui.add(myData, 'value3', 0, 20).listen();
...
function draw() {
  ...
  for (let y=img.height ; y>0; y-=10)
  {
    for (let x=img.width; x>0; x-=10)
    {
      ...
      {
        fill(myData.value0);
		    rect(x, y+150, myData.value3, myData.value3);
      }
      if (p < 200 & p > 100 )
      {
        fill(p,205,myData.value1);
        rect(x, y+150, myData.value3, myData.value3);
      }
      if (p < 100 & p > 0 )
      {
        fill(myData.value2,97,p);
        ellipse(x, y+150, myData.value3, myData.value3);
      }
    }
  }
  //drawGui();
